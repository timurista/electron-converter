...
export const addVideos = videos => dispatch => {
  ...
  ipcRenderer.on('meta:complete', (event, videosWithData) => {
    dispatch({
      type: ADD_VIDEOS,
...
export const convertVideos = () => (dispatch, getState) => {
  ...
  ipcRenderer.on('conversion:progress', (evt, {video, timemark}) => {
    dispatch({ type: VIDEO_PROGRESS, payload: {...video, timemark}})
  });
  ipcRenderer.on('conversion:end', (evt, {video, outputPath}) => {
